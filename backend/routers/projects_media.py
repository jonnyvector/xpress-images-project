...
from backend.models import ProjectResponse, SaveToFolderResponse
...
def save_results_to_folder(
    ...
    is_drawer = is_drawer_style(project.door_style)
...
def download_results_zip(
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        for wood_name, image_data in project.results:
            ...
            zf.writestr(
                filename,
                add_watermark(image_data, wood_name, watermark_offset, force_dark_text=use_dark)
                if watermark
                else image_data,
            )
    buf.seek(0)
